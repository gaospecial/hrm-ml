...
Amplification, melting curve and Ct standard curve of single species PCRs.
...
p_amp = lapply(seq_along(strain_label), function(i){
  ...
  ggplot(df, aes(cycle, delta_rn, group = well)) +
...
plot_grid(plotlist = p_mc, ncol = 3, nrow = 2)
```
...
plot_grid(plotlist = plots, align = 'hv', ncol = 3, nrow = 2)
```
...
plot_grid(plotlist = p_peak_height, ncol = 3, nrow = 2)
```
...
plot_grid(plotlist = p_peak_area, ncol = 3, nrow = 2)
```
...
plot_grid(plotlist = c(p_model_ct, p_model_mc), align = 'hv', ncol = 2, nrow = 2)
```
...
plot_grid(plotlist = p_mc_ml_cv_metric, ncol = 3, nrow = 2)
...
plot_grid(plotlist = c(p_lm_predict(), p_rf_predict()), ncol = 3, nrow = 2)
```
...
prop_gradient_metric = lapply(nums, function(n){
  lapply(seq_along(strain_label), function(i){
    # get train data
    train = training(data_split) |> 
      select(matches(strain_label[[i]]), starts_with("T")) |> 
      sample_n(n)
...
plot_grid(plotlist = p_grad_metric, ncol = 3, nrow = 2)
```
