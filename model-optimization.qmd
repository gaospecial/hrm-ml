...
rf_metric = function(train_data, 
  ...
  train_data = train_data |> select(any_of(y), X)
...
```{r}
rep2 = read.csv("data-clean/20230512.csv") |> 
...
rep3 = read.csv("data-clean/20230512.csv") |> 
  dplyr::filter(well_position %in% gradient_matrix_well,
                rep == 3,
                cycle == 30) |> 
  dplyr::select(starts_with('label_'), starts_with('T')) |> 
...
rf_rep_metric = lapply(prop, function(p){
  lapply(seq_along(strain_label), function(i){
    ...
    ylab = strain_label[[i]]
    train = training(data_split) |> select(any_of(ylab), starts_with("T"))
...
rate_metric = lapply(c(1, 2), function(r){
  ...
  rowwise() |> 
  mutate(rate = paste(rate_map[[rate]], "°C/s"))
...
plot_grid(
  ...
  plot_grid(plotlist = gglist, nrow = 1, rel_widths = c(1.2,1.2,1,1), 
            labels = letters[3:6]), 
  ncol = 1)
```
